...
@export var wander_length = 4.5
@export var wander_speed: DifficultyBasedFloat
@export var max_damaged_speed_multiplier: float = 3
...
func get_speed(zurg: Zurg) -> float:
	var val = wander_speed.value * lerpf(max_damaged_speed_multiplier, 1, zurg.damage.health.percentage)
	return val
...
func process(dt: float, zurg: Zurg):
	...
	zurg.position.z += move_direction * dt * get_speed(zurg)
